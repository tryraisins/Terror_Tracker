...
export default function DashboardPage() {
  ...
  const [chartView, setChartView] = useState<"incidents" | "impact">("incidents");
  ...
  return <div className="page-wrap">
    ...
    <section className="metric-grid" aria-label="Reported figures">
      ...
      <Metric label="People killed" value={overview.totalKilled} detail="reported count, estimate or midpoint" className="metric-card--impact" />
      <Metric label="People injured" value={overview.totalInjured} detail="reported count, estimate or midpoint" className="metric-card--caution" />
    ...
    <section className="dashboard-grid">
      <section className="panel"><div className="panel-heading"><div><h2>Most recorded states</h2><p className="panel-subtitle">Share of all {overview.year} incident records</p></div><Link href="/map" className="text-link">View map →</Link></div><div className="rank-list">{data.byState.slice(0, 5).map((item) => <div className="rank-row" key={item.state}><span>{item.state || "Unknown"}</span><span className="rank-row__track"><span className="rank-row__fill" style={{ width: `${(item.count / maxState) * 100}%` }} /></span><span className="rank-row__value"><strong>{item.count}</strong><small>{overview.totalAttacks ? `${((item.count / overview.totalAttacks) * 100).toFixed(1)}%` : "0%"}</small></span></div>)}</div></section>
