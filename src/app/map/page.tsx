...
export default function MapPage() {
  ...
  return <div className="page-wrap--wide">
    ...
    <section className="panel filter-panel"><div className="search-row"><label className="filter-label" style={{ flex: 1 }}>Evidence status<select className="control" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Any status</option><option value="confirmed">Confirmed</option><option value="developing">Developing</option><option value="unconfirmed">Unconfirmed</option></select></label><button className="button-quiet" type="button" onClick={() => { setStatus(""); setSelected(""); }}>Reset map filters</button></div></section>
    <section className="state-panel"><section className="panel map-shell"><div className="panel-heading"><div><h2>Incident distribution</h2><p className="panel-subtitle">Select a state marker to view its records</p></div></div><svg className="map-graphic" viewBox="0 0 1000 812" aria-hidden="true">{ranked.map((item) => <path key={item.id} className={`map-state ${active?.name === item.name ? "map-state--selected" : ""}`} d={item.path} />)}{ranked.filter((item) => item.count > 0).map((item) => { const radius = 12 + (item.count / max) * 19; const color = item.count / max > .7 ? "#7a1e28" : item.count / max > .35 ? "#a5373f" : "#9a6a20"; return <g key={`${item.id}-marker`} className="map-marker" onClick={() => setSelected(item.name)}><circle cx={item.x} cy={item.y} r={radius} fill={color} fillOpacity=".92" /><text x={item.x} y={item.y + 5} textAnchor="middle">{item.count}</text></g>; })}</svg><div className="map-legend"><span><i className="dot" style={{ background: "#f0eee9", border: "1px solid #d5d1c9" }} />0 records</span><span><i className="dot" style={{ background: "var(--caution)" }} />1–4</span><span><i className="dot" style={{ background: "var(--human-impact)" }} />5–7</span><span><i className="dot" />8+</span></div></section>
      <aside className="panel"><div className="panel-heading"><div><h2>Recorded states</h2><p className="panel-subtitle">Equivalent ranked list</p></div></div><div className="state-list">{ranked.map((item) => <button key={item.name} className="state-list__button" type="button" aria-pressed={active?.name === item.name} onClick={() => setSelected(item.name)}><span><i className="dot" style={{ background: item.count ? undefined : "#d5d1c9" }} />{item.name}</span><span className="state-list__count">{item.count} {item.count === 1 ? "record" : "records"}</span></button>)}</div></aside>
    </section>
    {active ? <section><div className="section-heading"><h2>Selected · {active.name}</h2><span className="chip chip--burgundy">{active.count} records</span></div><section className="panel summary-strip"><div><span className="eyebrow">Recorded human impact</span><div className="summary-strip__impacts"><span>{active.killed} killed</span><span>{active.kidnapped} abducted</span></div></div><div><span className="summary-strip__fine">Map describes the recorded dataset only.<br /><strong>It does not predict current safety.</strong></span><br /><Link className="text-link" href={`/incidents?state=${encodeURIComponent(active.name)}`}>Browse {active.name} records →</Link></div></section>{active.records.length ? <section className="record-list">{active.records.slice(0, 4).map((record) => <RecordCard key={record._id} incident={record} />)}</section> : <EmptyState title={`No ${status || "recorded"} incidents in ${active.name}`}>Choose another state or clear the current map filter.</EmptyState>}</section> : null}
  </div>;
}
