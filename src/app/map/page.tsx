...
export default function MapPage() {
  ...
  return <div className="page-wrap--wide">
    ...
    <section className="state-panel"><section className="panel map-shell"><div className="panel-heading"><div><h2>Recorded locations</h2><p className="panel-subtitle">Uniform markers identify stored record counts without ranking state prevalence.</p></div></div><svg className="map-graphic" viewBox="0 0 1000 812" aria-hidden="true">{stateSummaries.map((item) => <g key={item.id} className="map-state-group" onClick={() => setSelected(item.name)}><path className={`map-state ${active?.name === item.name ? "map-state--selected" : ""}`} d={item.path} /><title>{item.name}: {item.count} {item.count === 1 ? "record" : "records"}</title></g>)}{stateSummaries.map((item) => { const lines = labelLines(item.name); const radius = item.count ? 14 : 0; const labelY = item.y - Math.max(radius + 8, lines.length * 6); return <g key={`${item.id}-label`} className="map-label-group" onClick={() => setSelected(item.name)}><text className="map-state-label" x={item.x} y={labelY} textAnchor="middle">{lines.map((line, index) => <tspan x={item.x} dy={index === 0 ? 0 : 12} key={line}>{line}</tspan>)}</text>{item.count ? <g className="map-marker"><circle cx={item.x} cy={item.y} r={radius} fill="var(--burgundy)" fillOpacity=".96" /><text x={item.x} y={item.y + 5} textAnchor="middle">{item.count}</text></g> : null}</g>; })}</svg><div className="map-legend"><span><i className="dot map-legend__empty" />No stored records</span><span><i className="dot" style={{ background: "var(--burgundy)" }} />Uniform record-count marker</span></div></section>
      <aside className="panel state-list-panel"><div className="panel-heading"><div><h2>State record index</h2><p className="panel-subtitle">Alphabetical navigation; counts are not a risk ranking.</p></div></div><div className="state-list">{stateSummaries.map((item) => <button key={item.name} className="state-list__button" type="button" aria-pressed={active?.name === item.name} onClick={() => setSelected(item.name)}><span><i className="dot" style={{ background: item.count ? undefined : "var(--border-strong)" }} />{item.name}</span><span className="state-list__count">{item.count} {item.count === 1 ? "record" : "records"}</span></button>)}</div></aside>
    </section>
    {active ? <section><div className="section-heading"><h2>Selected · {active.name}</h2><span className="chip chip--burgundy">{active.count} records</span></div><section className="panel summary-strip"><div><span className="eyebrow">Impact recorded in this index</span><div className="summary-strip__impacts"><span>{active.killed} killed</span><span>{active.kidnapped} abducted</span></div></div><div><span className="summary-strip__fine">Counts omit unresolved candidates and cannot be compared as prevalence.<br /><strong>They do not predict current safety.</strong></span><br /><Link className="text-link" href={`/incidents?state=${encodeURIComponent(active.name === "Federal Capital Territory" ? "FCT" : active.name)}`}>Browse {active.name} records →</Link></div></section>{active.records.length ? <section className="record-list">{active.records.slice(0, 4).map((record) => <RecordCard key={record._id} incident={record} />)}</section> : <EmptyState title={`No ${status || "recorded"} incidents in ${active.name}`}>Choose another state or clear the current map filter.</EmptyState>}</section> : null}
  </div>;
}
